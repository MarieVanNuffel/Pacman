 ...
 * Het creëert entities via de abstract factory en notificeert de observers via
 * het Observer‑pattern. World is verantwoordelijk voor:
...
private:
  ...
  std::vector<std::vector<int>> maze; // 2D grid, 0 = empty, 1 = wall, 2 = coin,
                                      // 3 = fruit, 4 = pacman, 5 = ghost
  int mazeHeight;
  int mazeWidth;
...
public:
   ...
   */
  [[nodiscard]] const std::vector<std::shared_ptr<GhostModel>> &
  getGhosts() const {
    return ghosts;
  }
  /**
   ...
   */
  [[nodiscard]] const std::vector<std::vector<int>> &getMaze() const {
    return maze;
  };
  /**
   ...
   */
  [[nodiscard]] std::vector<std::shared_ptr<CoinModel>> getCoins() const {
    return coins;
  }
  /**
   ...
   */
  [[nodiscard]] std::vector<std::shared_ptr<FruitModel>> getFruits() const {
    return fruits;
  }
  /**
   ...
   */
  [[nodiscard]] std::vector<std::shared_ptr<GhostDoorModel>>
  getGhostDoors() const {
    return ghostDoors;
  }
  /**
  ...
  /**
   * @brief Spawnt de entities in de map. factory maakt views en observer worden
   * toegevoegd.
   *
   * Entities worden aangemaakt en in de maze gezet. view en observer worden
   * aangemaakt. Ghosts types worden bepaalt op de spawn volgorde.
   */
   ...
   * - Moeilijkheid wordt aangepast door snelheid te verhogen en kortere fear
   * mode.
   */
  void advanceLevel();
   ...
   * - Het verwerkt input buffers, verplaatst pacman en update ghosts, coins en
   * fruits.
   * - Het houdt de timeSinceLastcoin bij voor de score juist up te daten.
   * - Het kijkt of alle coins zijn verzameld en dus het level gedaan is en gaat
   * naar het volgende level.
   * - Als pacman een ghost aanraakt in CHASE mode wordt de death animatie
   * gestart.
   *
   ...
   *
   * Zet wereldcoördinaten om naar gridcoördinaten en kijkt of de tile een muur
   * is of buiten de map valt.
   *
   ...
   *
   * Kijkt naar muren en eventueel ghost doors. Het gebruikt de radius van de
   * entity.
   *
   * @param x X-positie in wereldcoördinaten.
   * @param y Y-positie in wereldcoördinaten.
   ...
   *
   * Zet wereldcoördinaten om naar gridcoördinaten en kijkt of de tile een
   * ghostdoor is.
   *
   * @param x X-positie in wereldcoördinaten.
   * @param y Y-positie in wereldcoördinaten.
   ...
   *
   * Kijkt met een kleine marge of we al dichtbij een tile center zitten om
   * uiteindelijk te zien of we van richting mogen veranderen.
   *
   * @param x X-positie in wereldcoördinaten.
   * @param y Y-positie in wereldcoördinaten.
   ...
   *
   * Kijkt of er 3 of meer vrije richtingen zijn vanaf een positie (en dus een
   * kruispunt).
   *
   * @param x X-positie in wereldcoördinaten.
   * @param y Y-positie in wereldcoördinaten.
   ...
   *
   * Berekent de volgende wereldpositie wanneer een entity één stap zou zetten
   * in de richting dat die nu is aan het gaan.
   *
   ...
   *
   * Gelijkaardig aan tryMoveEntity, maar met ghost-specifieke regels zoals
   * ghost doors en movement states.
   *
   ...
   *
   * Gebruikt breadth first om een richtingen te bepalen van startpositie
   * (sx,sy) naar target (tx,ty).
   *
   * Wordt voornamelijk gebruikt om ghosts terug naar spawn te laten gaan
   * wanneer ze opgegeten worden.
   *
   ...
   */
  std::vector<Direction> findPath(int sx, int sy, int tx, int ty,
                                  bool allowDoor = false) const;
};
} // namespace logic

...
GhostModel::GhostModel(GhostType t) : type(t) {
  ...
  // Start Direction afhankelijk van ghost type (ghosts kijken naar elkaar in
  // box)
...
void GhostModel::update(double dt) {
  ...
  if (!worldPtr)
    return;
  if (frozen)
    return; // geen beweging of updates timers wanneer bevroren
  ...
  if (mode == Mode::Eaten) {
    ...
    bool nearCenter = (std::abs(x - snapX) < snapEpsilon &&
                       std::abs(y - snapY) < snapEpsilon);

    // Zorgen dat de ghosts in het midden van de gang lopen
    double centerForceEpsilon = 0.20;
    if (direction == Direction::UP || direction == Direction::DOWN) {
      if (std::abs(x - snapX) < centerForceEpsilon) {
        x = snapX;
        setPosition(x, y);
      }
    } else if (direction == Direction::LEFT || direction == Direction::RIGHT) {
      if (std::abs(y - snapY) < centerForceEpsilon) {
        y = snapY;
        setPosition(x, y);
      }
    }
    ...
    if (worldPtr)
      path =
          worldPtr->findPath(csx, csy, tx, ty, true); // ghostdoor is toegestaan
    ...
    if (!path.empty()) {
      ...
      if (isOpposite(cand, direction) && path.size() > 1) {
        std::vector<Direction> viable =
            worldPtr->getFreeDirections(currentX, currentY);
        for (Direction alt : viable) {
          if (!isOpposite(alt, direction)) {
            cand = alt;
            break;
          }
        }
  ...
  double snapEpsilon = 0.15;
  bool nearCenter =
      (std::abs(x - snapX) < snapEpsilon && std::abs(y - snapY) < snapEpsilon);
  ...
  // gebruik gecentreerde coords voor Free Directions wanneer we dichtbij tile
  // center zijn
  double checkX = nearCenter ? snapX : x;
  double checkY = nearCenter ? snapY : y;

  std::vector<Direction> viableDirs =
      worldPtr->getFreeDirections(checkX, checkY);
  if (viableDirs.empty()) {
    return;
  }
  ...
  if (type == GhostType::LockedRandom) {
    if (logic::Random::instance().nextDouble(0.0, 1.0) <
        0.5) { // 50% kans op nieuwe richting
      Direction cand = viableDirs[logic::Random::instance().nextInt(
          0, static_cast<int>(viableDirs.size()) - 1)];
      if (isOpposite(cand, direction) &&
          viableDirs.size() > 1) { // haal de 180 graden eruit als mogelijk
        std::vector<Direction> nonOpp;
        for (Direction d : viableDirs)
          if (!isOpposite(d, direction))
            nonOpp.push_back(d);
        if (!nonOpp.empty()) {
          cand = nonOpp[logic::Random::instance().nextInt(
              0, static_cast<int>(nonOpp.size()) - 1)];
        }
      }
      chosen = cand;
    } else {
  ...
  bool maximize =
      (mode ==
       Mode::Fear); // in fear mode wordt de manhatten afstand gemaximaliseert
  ...
  if (maximize) {
    ...
    if (type == GhostType::AheadOfPacman1 ||
      ...
      std::tie(refX, refY) = worldPtr->predictStep(
          pm->getX(), pm->getY(),
          pm->getDirection()); // aheadOfPacman gebruikt tile voor pacman als
                               // target
  ...
  for (Direction d : finalDirs) {
    auto [ghostNextX, ghostNextY] =
        worldPtr->predictStep(currentX, currentY, d);
    ...
    if (maximize) {
      ...
      } else if (std::abs(metric - bestMetric) < 1e-9) {
        bestDirs.push_back(d);
      }
    } else {
      if (metric < bestMetric - 1e-9) {
  ...
  if (!bestDirs.empty()) {
    Direction cand = bestDirs[logic::Random::instance().nextInt(
        0, static_cast<int>(bestDirs.size()) - 1)];

    // Liefst geen 180 graden draai
    if (!maximize && isOpposite(cand, direction) && bestDirs.size() > 1) {
      std::vector<Direction> nonOpp;
      for (Direction d : bestDirs)
        if (!isOpposite(d, direction))
          nonOpp.push_back(d);
      if (!nonOpp.empty())
        cand = nonOpp[logic::Random::instance().nextInt(

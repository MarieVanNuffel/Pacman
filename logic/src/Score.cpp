...
void Score::coinCollected(double timeSinceLastCoin) {
  ...
  if (timeSinceLastCoin < 0.5)
    points = 50;
  else if (timeSinceLastCoin < 1.0)
    points = 30;
  else if (timeSinceLastCoin < 2.0)
    points = 20;
...
void Score::pacmanDied() { currentScore -= 100; }

void Score::levelCleared() { currentScore += 1000; }

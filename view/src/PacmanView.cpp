...
void PacmanView::onNotify(int event) {
  if (event == static_cast<int>(logic::Event::PACMAN_DIED))
    startDeath();
  else if (event == static_cast<int>(logic::Event::PACMAN_REVIVED))
    stopDeath();
}
...
void PacmanView::updateSprite(double dt) {
  ...
  if (deathPlaying) {
    ...
    if (animTimer >= frameTime) {
      ...
      if (deathFrameIndex >= NUM_DEATH_FRAMES) {
        deathFrameIndex =
            NUM_DEATH_FRAMES -
            1; // De laatste frame wordt vastgehouden tot de reset gebeurd
      }
    }
  ...
  // als pacman tegen een muur loopt, dan moet de huidige direction behouden
  // worden
  ...
  case logic::Direction::DOWN:
    ...
    break;
  default:
    return; // geen beweging, dus animatie niet updaten
  }
  sprite.setTextureRect(sf::IntRect(frameX, frameY, FRAME_SIZE, FRAME_SIZE));
}
} // namespace view
